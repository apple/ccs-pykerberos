...
int authenticate_gss_sign(
    ...
    OM_uint32 maj_stat;
    OM_uint32 min_stat;
    gss_buffer_desc input_message = GSS_C_EMPTY_BUFFER;
    gss_buffer_desc output_token = GSS_C_EMPTY_BUFFER;
    int ret = AUTH_GSS_CONTINUE;

    if (message && *message) {
        size_t len;
        input_message.value = base64_decode(message, &len);
    ...
    if (output_token.length && token) {
        *token = base64_encode((const unsigned char *)output_token.value, output_token.length);
        if (*token == NULL) {
            PyErr_NoMemory();
            ret = AUTH_GSS_ERROR;
            goto end;
...
int authenticate_gss_verify(
    ...
    if (token && *token) {
        size_t len;
        input_token.value = base64_decode(token, &len);
